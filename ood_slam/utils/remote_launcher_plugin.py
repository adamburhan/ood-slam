...
# from https://github.com/mila-iqia/ResearchTemplate
...
class RemoteSlurmLauncher(BaseSubmititLauncher):
    ...
    def launch(
        ...
        # Safe result retrieval that handles CUDA tensors on CPU-only machines
        results = []
        for j in jobs:
            try:
                results.append(j.results()[0])
            except RuntimeError:
                # Load result with CPU mapping for CUDA tensors
                import torch

                # Monkey patch pickle load to use CPU mapping
