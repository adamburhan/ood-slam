# from https://github.com/ChiWeiHsiao/DeepVO-pytorch/blob/master/preprocess.py
import os
import glob
import numpy as np
import time
from data.utils import R_to_angle
...
# Use package-based configs so Hydra can load them when running on remote nodes.
import ood_slam
import ood_slam.configs  # triggers config registration
...
def clean_unused_images(image_dir):
	...
	for dir_id, img_ids in seq_frame.items():
		dir_path = os.path.join(image_dir, dir_id)
...
def main(cfg: DictConfig):
    if cfg.preprocessing.clean_images:
        clean_unused_images(cfg.data.raw_data_dir + "/images/")
        
    if cfg.preprocessing.create_poses:
        create_pose_data(cfg.data.raw_data_dir + "/poses/")
        
    if cfg.preprocessing.calculate_stats:
        image_path_list = []
        for folder in cfg.preprocessing.train_sequences:
            image_path_list += glob.glob(f"{cfg.data.raw_data_dir}/images/{folder}/*.png")
        stats = calculate_rgb_mean_std(image_path_list, minus_point_5=True)
